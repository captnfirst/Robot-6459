...
public class Robot extends TimedRobot {
  ...
  double speedArmPosThreshold = 10000;
  ...
  public void teleopPeriodic() {
    ...
    if (desiredPos == -1) { // human player
      kDriveSpeed = 0.5;
      kRotationSpeed = 0.5;
    } else if (desiredPos == 0) { // transport
      kDriveSpeed = 0.87;
      kRotationSpeed = 0.56;
    } else if (desiredPos == 1) { // 1lvl
      kDriveSpeed = 0.78;
      kRotationSpeed = 0.54;
    } else if (desiredPos == 2 || desiredPos == 3) { // 2&3 lvl
      kDriveSpeed = 0.5;
      kRotationSpeed = 0.52;
    } else if (desiredPos == 4) { // ground
      kDriveSpeed = 0.48;
      kRotationSpeed = 0.48;
    }

    if (yardimciJoystick.getRawButton(2)) {
      desiredPos = 0; // transport
    ...
    if (yardimciJoystick.getRawButton(5)) {
      m_robotContainer.m_intake.setIntakeSolenoid(Value.kReverse); // on
    } else if (yardimciJoystick.getRawButton(6)) {
      m_robotContainer.m_intake.setIntakeSolenoid(Value.kForward); // off
    }

    if (yardimciJoystick.getRawButton(7)) {
      m_robotContainer.m_intake.intakeMotor.set(0.3);
    } else if (yardimciJoystick.getRawButton(8)) {
      if (desiredPos == 3 && gamePiece == -1) { // 3lvl cube
        m_robotContainer.m_intake.setIntake(-0.72);
      } else if (desiredPos == 3 && gamePiece == 1) { // 3lvl cone
        ...
        if (m_robotContainer.m_turret.getTurretPos() > -40 && m_robotContainer.m_turret.getTurretPos() < 40) {
          ...
          m_robotContainer.m_intake.intakeMotor.set(-1);
        }
      } else if (desiredPos == 1 && gamePiece == 1) { // 1lvl cone
        if (m_robotContainer.m_turret.getTurretPos() > -40 && m_robotContainer.m_turret.getTurretPos() < 40) {
          m_robotContainer.m_intake.intakeMotor.stopMotor();
        } else {
          m_robotContainer.m_intake.intakeMotor.set(-0.5);
        }
      } else {
    ...
    if (yardimciJoystick.getPOV() == 90 && (desiredPos == 2 || desiredPos == 3)) {
    ...
    if (backlashTimer.get() > 2) {
      if (desiredPos == -1) {
        m_robotContainer.m_elevator.setElevator(true, -0.5);
        if (m_robotContainer.m_elevator.getElevatorPos() > -5) {
          m_robotContainer.m_arm.setArm(true, 101.5 + armAngleOffset_humanPlayer); // 85 - 100
          if (m_robotContainer.m_arm.getArmPos() > 30) {
            m_robotContainer.m_head.setHead(true, -6000);
          }
        }
      } else if (desiredPos == 0) { // Transport
        if (m_robotContainer.m_arm.getArmPos() > 40) {
          m_robotContainer.m_arm.setArm(true, 22.5);
          m_robotContainer.m_head.setHead(true, 8500);
          if (m_robotContainer.m_arm.getArmPos() < 25) {
            ...
            if (m_robotContainer.m_turret.getTurretPos() > 170 && m_robotContainer.m_turret.getTurretPos() < 190) {
              if (m_robotContainer.m_arm.getArmPos() > 20) {
                m_robotContainer.m_elevator.setElevator(true, -32.5);
              }
            }
          }
        } else {
      ...
      } else if (desiredPos == 1) { // 1 lvl
        m_robotContainer.m_elevator.setElevator(true, -0.5);
        if (m_robotContainer.m_elevator.getElevatorPos() > -5) {
          m_robotContainer.m_arm.setArm(true, -5);
          ...
          if (m_robotContainer.m_arm.getArmPos() < 20) {
            m_robotContainer.m_head.setHead(true, 10500);
          }
        }
        m_robotContainer.m_turret.lockOnTarget(false, false);
        ...
        if (m_robotContainer.m_elevator.getElevatorPos() > -5) {
          ...
          if (m_robotContainer.m_arm.getArmPos() > 30) {
            // m_robotContainer.m_head.setHead(true, -11500);
            // m_robotContainer.m_head.setHead(true, -16300);
            // m_robotContainer.m_head.setHead(true, -18000);
            m_robotContainer.m_head.setHead(true, -16300);
          }
        }
      } else if (desiredPos == 3) { // 3lvl
        m_robotContainer.m_elevator.setElevator(true, -0.5);
        if (m_robotContainer.m_elevator.getElevatorPos() > -5) {
          m_robotContainer.m_turret.lockOnTarget(true, m_robotContainer.m_turret.visionLockingStatus);
          m_robotContainer.m_arm.setArm(true, 120);
